...
<?php 
	...
	$query = "SELECT * FROM order_info WHERE is_deleted=0 ORDER BY order_info_id";
	$users = mysqli_query($connection, $query);

	verify_query($users);
	while ($user = mysqli_fetch_assoc($users)) {
		$user_list .= "<tr>";
		$user_list .= "<td>{$user['order_info_id']}</td>";
		$user_list .= "<td>{$user['customer_id']}</td>";
		$user_list .= "<td>{$user['first_name']}</td>";
		$user_list .= "<td>{$user['last_name']}</td>";
		$user_list .= "<td>{$user['email']}</td>";
		$user_list .= "<td>{$user['total_amount']}</td>";


		$user_list .= "<td><a href=\"delete-product.php?order_info_id={$user['order_info_id']}\">Delete</a></td>";
		$user_list .= "</tr>";
	}
 ?>
<!DOCTYPE html>
...
<div class="content">
	...
	<main>
		<h1 style="font-size: 40px;">Order Details</h1>
